...
// trait LoginIntegrationHelper
//     extends RegistrationIntegrationHelper
...
//   def frontpageWithSession(session: Option[String]) =
//     wsWithSession(frontUrl, session).get()
// }

// trait CookieIntegrationHelper {

//   val secretKey =
//     Configuration.load(Environment.simple()).get[String]("play.http.secret.key")
...
//   private def findCookie(
//       response: WSResponse,
//       cookieName: String
//   ): Option[WSCookie] =
//     response.cookies.find(_.name == cookieName)

//   private def findDecodedCookie(
//       response: WSResponse,
//       cookieName: String
...
//   def findFlashCookie(response: WSResponse, key: String): Option[String] =
//     findDecodedCookie(response, "PLAY_FLASH").map(_(key))

// }

// class LoginIntegrationSpec
...
//         And("not logged in")
//         val loginFormBefore = ScalaSoup
//           .parse(frontBefore.body)
//           .select("#login-box input")
//           .headOption
...
//         val loginFormAfter =
//           ScalaSoup.parse(frontAfter.body).select("#login-box input").headOption
//         loginFormAfter shouldBe None
//         val logoutLink =
//           ScalaSoup.parse(frontAfter.body).select("#logout-box li a").headOption
...
//         val logoutLink =
//           ScalaSoup.parse(frontLogin.body).select("#logout-box li a").headOption
...
//           logoutResponse,
//           "message"
//         ).value shouldBe "You have been logged out"

//         Then("should be logged out")
//         val loggedOutLink = ScalaSoup
//           .parse(frontLogout.body)
//           .select("#logout-box li a")
//           .headOption
